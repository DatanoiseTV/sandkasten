...
> **No private key material lives in this repo.** The release
> workflow uses sigstore *keyless* signing — every release is signed
> by a short-lived certificate issued by the public sigstore CA based
> on the GitHub Actions OIDC token, and the certificate is discarded
> within minutes of the run. There is no `cosign.key`, no GPG private
> key, no minisign private key checked in or generated by CI. The
> `.gitignore` additionally denies `*.key` / `*.sec` / `*.pem` /
> `*.priv` / `*.gpg` / `id_*` patterns as a backstop against
